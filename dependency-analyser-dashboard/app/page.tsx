...
export default function Home() {
    ...
    // Initial fetch: small/global datasets only — no row-limit risk
    useEffect(() => {
        Promise.all([
    ...
    // Per-project fetch: re-runs whenever the selected project changes
    useEffect(() => {
        if (!selectedProject) return;
        setDetailRows([]);
        setFilters({ riskTier: null, libraryKey: null });
        supabase
            .from('v_dependency_detail')
            .select('*')
    ...
    // detailRows is already scoped to selectedProject by the fetch query; org filter is the only
    // remaining dimension needed here (a project belongs to one org, so this is mostly a safeguard)
    const projectFilteredRows = useMemo(() =>
            selectedOrg ? detailRows.filter(r => r.org_name === selectedOrg) : detailRows,
        [detailRows, selectedOrg]
    );
    ...
    // Detail table: apply cross-filters; project is already scoped by the fetch
    const filteredDetail = useMemo(() =>
            projectFilteredRows.filter(r => {
                if (filters.riskTier && r.risk_tier !== filters.riskTier) return false;
                if (filters.libraryKey && `${r.group_id}:${r.artifact_id}` !== filters.libraryKey) return false;
                return true;
            }),
        [projectFilteredRows, filters]
    );
